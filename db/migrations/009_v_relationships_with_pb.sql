-- Migration 009: extend v_relationships with PitchBook-sourced fields.
-- PB description, AUM, total investments, and the primary-contact metadata
-- (so the Streamlit cards have richer info). Prefers PB description over
-- Apollo/scraped descriptions because it's typically the most accurate.
...
SELECT
  ...
  f.type                                 AS firm_type,
  f.domain,
  COALESCE(f.apollo_website, f.pb_website, f.url)    AS url,
  f.hq_city,
  f.hq_country,
  ...
  COALESCE(f.aum_usd_m, f.pb_aum_usd_m)  AS aum_usd_m,
  f.fund_size_usd_m,
  -- Prefer PitchBook description, then Apollo, then scraped strategy
  COALESCE(f.pb_description, f.apollo_description, f.strategy) AS firm_strategy,
  f.enrichment_status,
  f.name_aliases                         AS firm_aliases,

  -- Structured mandate fields
  f.accepts_debt,
  f.accepts_equity,
  ...
  f.pb_description,
  f.pb_aum_usd_m,
  f.pb_total_investments,
  f.pb_active_portfolio,
  f.pb_investor_type,
  f.pb_hq_location,
  f.pb_preferred_industries,
  f.pb_preferred_investment_types,
  f.pb_website,
  f.pb_primary_contact_name,
  f.pb_primary_contact_title,
  f.pb_primary_contact_email,
  f.pb_primary_contact_phone,
  f.pb_check_size_min_usd_m,
  f.pb_check_size_max_usd_m,
  f.pb_provenance,
...
INSERT OR IGNORE INTO schema_migrations(version) VALUES ('009_v_relationships_with_pb');
